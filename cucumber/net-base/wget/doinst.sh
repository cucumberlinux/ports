...
# Remove the wgetrc from any existing wget package file list. This prevents an old wgetrc (which was not
# installed as a .new file) from being removed when upgrading the package.
sed -i var/log/packages/wget-* -e "/etc\\/wgetrc/d"

install_new_file etc/wgetrc
