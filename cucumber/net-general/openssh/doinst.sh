...
# OpenSSH needs the user and group sshd to exist for the SSH daemon
# Add them if they don't already exist
if [ -z "$(grep sshd: etc/group)" ]; then
	echo "sshd:x:50:sshd" >> etc/group
fi
if [ -z "$(grep sshd: etc/passwd)" ]; then
	echo "sshd:x:50:50:SSH Daemon:/var/lib/sshd:/bin/false" >> etc/passwd
fi
if [ -z "$(grep sshd: etc/shadow)" ]; then
	echo "sshd:!:17149:0:::::" >> etc/shadow
fi

install_new_file etc/ssh/ssh_config
install_new_file etc/ssh/sshd_config
